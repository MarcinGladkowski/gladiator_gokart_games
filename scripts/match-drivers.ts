...
const drivers: string[] = JSON.parse(
...
// Each row: data-member_id="X" ... <td class="pseudo">NICKNAME</td>
const rowRegex = /data-member_id="(\d+)"[\s\S]*?<td class="pseudo">([^<]+)<\/td>/g;

const allDrivers: { memberId: string; nickname: string }[] = [];
...
const matched: { nickname: string; memberId: string }[] = [];
